...
# GUI setup
root = tk.Tk()
root.title("Talking Assistant")

canvas = tk.Canvas(root, width=400, height=400)
canvas.pack()

# Load images for the talking face
mouth_open_image = PhotoImage(file="C:/Users/Admin/Downloads/mouth_open.png")
mouth_closed_image = PhotoImage(file="C:/Users/Admin/Downloads/mouth_closed.png")
eye_image = PhotoImage(file="C:/Users/Admin/Downloads/eye.png")

# Display the talking face
eye = canvas.create_image(200, 150, image=eye_image)
mouth = canvas.create_image(200, 250, image=mouth_closed_image)

def update_face(is_open):
    canvas.itemconfig(mouth, image=mouth_open_image if is_open else mouth_closed_image)
...
def process_command(command):
    ...
    if command:
        ...
        if "time" in command:
            ...
            response = solve_math(math_operation)
    ...
    if os.path.exists(notes_file):
        with open(notes_file, "r") as file:
            ...
            for i, note in enumerate(notes):
                note_text = f"Note {i + 1}: {note['note']} taken on {note['timestamp']}"
                speak(note_text)
...
def update_gui(response):
    global root, canvas
    root.deiconify()
    canvas.itemconfig(mouth, image=mouth_open_image)
    root.update()
    speak(response)
    canvas.itemconfig(mouth, image=mouth_closed_image)
    root.update()
